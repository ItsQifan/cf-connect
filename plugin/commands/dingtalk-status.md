...
2. `cf-connect config format --config "%USERPROFILE%\.cf-connect\config.toml"` —— 确认配置能解析
   （配置固定放在用户目录：`%USERPROFILE%\.cf-connect\config.toml`，和会话历史/日志/socket 同处）
...
| 第 4 条没有 `api.sock` | 网关没启动（最常见：只用了前台方式跑，命令结束进程就没了） | `cf-connect daemon install --config "%USERPROFILE%\.cf-connect\config.toml"`（装过就 `daemon restart`），再 `cf-connect daemon status` 确认 running |
