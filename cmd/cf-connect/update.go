...
const (
	...
	giteeAPI     = "https://gitee.com/api/v5/repos/cg33/cf-connect/releases/latest"
)
...
func getUpdateHintIfAvailable() string {
	...
	if isNewer(cachedVer, version) {
		return fmt.Sprintf("\n📦 Update available: %s → %s  (run: cf-connect update)\n", version, cachedVer)
	}
	return ""
}
...
func runUpdate() {
	...
	fmt.Println("Restart cf-connect to use the new version.")
}
...
func binaryAssetName(tag string) string {
	...
	name := fmt.Sprintf("cf-connect-%s-%s-%s", tag, goos, goarch)
	if goos == "windows" {
		name += ".exe"
	}
	return name
...
func archiveAssetName(tag string) string {
	goos := runtime.GOOS
	goarch := runtime.GOARCH
	base := fmt.Sprintf("cf-connect-%s-%s-%s", tag, goos, goarch)
	if goos == "windows" {
		return base + ".zip"
	}
	return base + ".tar.gz"
...
// extractBinaryFromArchive extracts the cf-connect binary from a .tar.gz or .zip archive.
...
func extractFromTarGz(archivePath string) (string, error) {
	...
	for {
		...
		if strings.HasPrefix(hdr.Name, "cf-connect") {
			tmp, err := os.CreateTemp("", "cf-connect-update-*")
			if err != nil {
				return "", err
			}
...
func extractFromZip(archivePath string) (string, error) {
	...
	for _, f := range r.File {
		if !strings.HasPrefix(f.Name, "cf-connect") {
			continue
		}
		rc, err := f.Open()
		if err != nil {
...
func checkUpdate() {
	...
	if isNewer(release.TagName, version) {
		hint := "cf-connect update"
		if release.Prerelease {
			hint = "cf-connect update --pre"
		}
...
// (node_modules/cf-connect/bin/) and updates the package.json version to
...
func syncNpmPackageVersion(execPath, newVer string) {
	...
	if err := os.WriteFile(pkgJSON, out, 0o644); err != nil {
		...
		fmt.Println("   please run: npm update -g cf-connect")
	} else {
