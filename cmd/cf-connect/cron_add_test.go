...
// `cf-connect cron add --silent` flag. The flag is parsed in runCronAdd
// (cmd/cf-connect/cron.go) and sets body["silent"] = true on the /cron/add
// request; if printCronAddUsage ever drifts and stops mentioning it, users
// who run `cf-connect cron add --help` will not discover the feature even
...
func TestPrintCronAddUsage_DocumentsSilent(t *testing.T) {
	...
	if !strings.Contains(help, "cf-connect cron add --cron \"0 9 * * *\" --prompt \"Daily standup reminder\" --silent") {
