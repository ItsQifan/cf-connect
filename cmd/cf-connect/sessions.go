...
func runSessions(args []string) {
	...
	case "show":
		...
		if id == "" {
			...
			fmt.Fprintln(os.Stderr, "Usage: cf-connect sessions show <session-id> [-n N]")
			os.Exit(1)
		}
...
func resolveDataDir(flagValue string) string {
	...
	if home, err := os.UserHomeDir(); err == nil {
		return filepath.Join(home, ".cf-connect")
	}
	return ".cf-connect"
}
...
func runSessionsShow(dataDir, id string, limit int) {
	...
	if record == nil {
		...
		fmt.Fprintln(os.Stderr, "Use 'cf-connect sessions list' to see available sessions.")
		os.Exit(1)
	}
...
func printSessionsUsage() {
	fmt.Println(`Usage: cf-connect sessions [command] [options]
...
Session ID formats for 'show':
  <project>:<session>   e.g. "dingtalk-smoke:s1"
...
Examples:
  cf-connect sessions                           Interactive TUI browser
  cf-connect sessions list                      List all sessions
  cf-connect sessions show "mybot:s1"           Show all messages in session
  cf-connect sessions show "#1" -n 20           Show last 20 messages of first session
  cf-connect sessions prune                     Remove empty duplicate sessions
  cf-connect sessions prune --empty             Same as above, explicit form
  cf-connect sessions prune --merge             Merge duplicates, keeping most recent
  cf-connect sessions prune mybot --merge       Prune specific project`)
}
