...
func daemonInstall(args []string) {
	...
	fmt.Println("cf-connect daemon installed and started.")
	fmt.Println()
	...
	fmt.Println("  cf-connect daemon status    - Check status")
	fmt.Println("  cf-connect daemon logs -f   - Follow logs")
	fmt.Println("  cf-connect daemon restart   - Restart")
	fmt.Println("  cf-connect daemon stop      - Stop")
	fmt.Println("  cf-connect daemon uninstall - Remove")
	...
	if strings.Contains(mgr.Platform(), "user") {
		...
		if !enabled {
			...
			fmt.Println("   cf-connect will stop when your last login session ends (e.g., SSH disconnect).")
...
func daemonUninstall() {
	...
	daemon.RemoveMeta()
	fmt.Println("cf-connect daemon uninstalled.")
}
...
func daemonStart() {
	...
	fmt.Println("cf-connect daemon started.")
}
...
func daemonStop() {
	...
	fmt.Println("cf-connect daemon stopped.")
}
...
func daemonRestart(args []string) {
	...
	fmt.Println("cf-connect daemon restarted.")
}
...
func requireInstalled(mgr daemon.Manager) {
	...
	if st == nil || !st.Installed {
		...
		fmt.Fprintln(os.Stderr, "  cf-connect daemon install --work-dir /path/to/config-dir")
		os.Exit(1)
	}
}
...
func daemonStatus() {
	...
	fmt.Println("cf-connect daemon status")
	fmt.Println()
	...
	if !st.Installed {
		...
		fmt.Printf("  Platform:  %s\n", st.Platform)
		fmt.Println()
		fmt.Println("  Run: cf-connect daemon install")
		return
	}
...
func printDaemonUsage() {
	fmt.Println(`Usage: cf-connect daemon <command> [flags]
...
Install flags:
  ...
  --log-file PATH       Log file path (default: ~/.cf-connect/logs/cf-connect.log)
