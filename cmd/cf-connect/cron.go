...
func runCronAdd(args []string) {
	...
	// Fallback to env vars (set by cf-connect when spawning agent)
	if project == "" {
	...
	if _, err := os.Stat(sockPath); os.IsNotExist(err) {
		fmt.Fprintf(os.Stderr, "Error: cf-connect is not running (socket not found: %s)\n", sockPath)
		os.Exit(1)
	}
...
func runCronInfo(args []string) {
	...
	if id == "" {
		fmt.Fprintln(os.Stderr, "Error: job ID is required")
		fmt.Fprintln(os.Stderr, "Usage: cf-connect cron info <id> [field]")
		fmt.Fprintln(os.Stderr, "Use 'cf-connect cron list' to see all task IDs.")
		os.Exit(1)
	}
...
func runCronEdit(args []string) {
	...
	if id == "" {
		fmt.Fprintln(os.Stderr, "Error: job ID is required")
		fmt.Fprintln(os.Stderr, "Run 'cf-connect cron edit --help' for usage.")
		os.Exit(1)
	}
	if field == "" {
...
// documented as a bool in printCronEditUsage, so `cf-connect cron edit <id>
...
func printCronUsage() {
	fmt.Println(`Usage: cf-connect cron <command> [options]
...
Run 'cf-connect cron <command> --help' for details.`)
}

func printCronAddUsage() {
	fmt.Println(`Usage: cf-connect cron add [options] [<min> <hour> <day> <month> <weekday> <prompt>]
...
Options:
  ...
  -c, --cron <expr>          Cron expression, e.g. "0 6 * * *"
      ...
      --data-dir <path>      Data directory (default: ~/.cf-connect)
  -h, --help                 Show this help

Examples:
  cf-connect cron add --cron "0 6 * * *" --prompt "Collect GitHub trending data" --desc "Daily Trending"
  cf-connect cron add --cron "*/30 * * * *" --exec "df -h" --desc "Disk usage check"
  cf-connect cron add --cron "0 9 * * *" --prompt "Daily standup reminder" --silent
  cf-connect cron add 0 6 * * * Collect GitHub trending data and send me a summary`)
}

func printCronExecUsage() {
	fmt.Println(`Usage: cf-connect cron exec <id> [options]
...
Example:
  ...
  cf-connect cron run abc123`)
}

func printCronEditUsage() {
	fmt.Println(`Usage: cf-connect cron edit <id> <field> <value> [options]
...
Examples:
  ...
  cf-connect cron edit abc123 description "Daily standup reminder"
  cf-connect cron edit abc123 timeout_mins 60
  cf-connect cron edit abc123 mute true`)
}
