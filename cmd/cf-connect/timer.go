...
func runTimerInfo(args []string) {
	...
	if id == "" {
		fmt.Fprintln(os.Stderr, "Error: timer ID is required")
		fmt.Fprintln(os.Stderr, "Usage: cf-connect timer info <id>")
		os.Exit(1)
	}
...
func printTimerUsage() {
	fmt.Println(`Usage: cf-connect timer <command> [options]
...
Run 'cf-connect timer <command> --help' for details.`)
}

func printTimerAddUsage() {
	fmt.Println(`Usage: cf-connect timer add [options] <delay> <prompt>
...
Examples:
  cf-connect timer add --delay 2h --prompt "Check PR status"
  ...
  cf-connect timer add 2h Check PR status`)
}
