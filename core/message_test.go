...
func TestSaveFilesToDisk_RejectsPathTraversal(t *testing.T) {
	workDir := t.TempDir()
	attachDir := filepath.Join(workDir, ".cf-connect", "attachments")

	files := []FileAttachment{
		// The original repro: walks two levels up out of attachments and
		// out of .cf-connect/, landing directly in workDir.
...
// regression from issue #1459: when workDir is relative (e.g. ".cf-connect"
...
// process whose cwd may differ from cf-connect's.
...
// regression for users who already configured cf-connect with absolute
...
func TestSaveFilesToDisk_EmptyWorkDirFallsBackToCwd(t *testing.T) {
	...
	wantPrefix := filepath.Join(cwd, ".cf-connect", "attachments") + string(filepath.Separator)
...
func TestSaveFilesToDisk_SameNameInOneCall(t *testing.T) {
	...
	entries, err := os.ReadDir(filepath.Join(workDir, ".cf-connect", "attachments", "message-1"))
	if err != nil {
...
func TestSaveFilesToDisk_NoSubdirBackwardsCompat(t *testing.T) {
	...
	wantPath := filepath.Join(workDir, ".cf-connect", "attachments", "legacy.txt")
	if first[0] != wantPath {
