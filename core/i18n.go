...
//
// All exported methods are safe to call from multiple goroutines: cf-connect
...
// without duplicating the switch statement that cmd/cf-connect/main.go
...
const (
	...
	// so that operators running cf-connect with language="zh" see the
...
var messages = map[MsgKey]map[Language]string{
	...
	MsgUnknownCommand: {
		LangEnglish:            "`%s` is not a cf-connect command, forwarding to agent...",
		LangChinese:            "`%s` 不是 cf-connect 命令，已转发给 Agent 处理...",
		LangTraditionalChinese: "`%s` 不是 cf-connect 命令，已轉發給 Agent 處理...",
		LangJapanese:           "`%s` は cf-connect のコマンドではありません。エージェントに転送します...",
		LangSpanish:            "`%s` no es un comando de cf-connect, reenviando al agente...",
	},
	MsgWelcome: {
		LangEnglish:            "👋 Hi! I'm cf-connect, bridging you to **%s**.\n\nJust send a message to chat with the agent. Type /help to see built-in commands.",
		LangChinese:            "👋 你好！我是 cf-connect，已为你连接到 **%s**。\n\n直接发送消息即可与 Agent 对话。输入 /help 查看内置命令。",
		LangTraditionalChinese: "👋 你好！我是 cf-connect，已為你連接到 **%s**。\n\n直接發送訊息即可與 Agent 對話。輸入 /help 查看內建命令。",
		LangJapanese:           "👋 こんにちは！cf-connect が **%s** に接続しました。\n\nメッセージを送信すればエージェントと会話できます。/help で組み込みコマンド一覧を確認できます。",
		LangSpanish:            "👋 ¡Hola! Soy cf-connect, conectándote con **%s**.\n\nEnvía un mensaje para chatear con el agente. Usa /help para ver los comandos integrados.",
	},
	MsgHelp: {
		LangEnglish: "📖 Available Commands\n\n" +
			...
			"/restart\n  Restart cf-connect service\n\n" +
			"/status\n  Show system status\n\n" +
			"/version\n  Show cf-connect version\n\n" +
		...
		LangSpanish: "📖 Comandos disponibles\n\n" +
			...
			"/restart\n  Reiniciar el servicio cf-connect\n\n" +
			"/status\n  Mostrar estado del sistema\n\n" +
			"/version\n  Mostrar versión de cf-connect\n\n" +
	...
	MsgHelpTitle: {
		LangEnglish:            "cf-connect Help",
		LangChinese:            "cf-connect 帮助",
		LangTraditionalChinese: "cf-connect 說明",
		LangJapanese:           "cf-connect ヘルプ",
		LangSpanish:            "cf-connect Ayuda",
	},
	...
	MsgProviderListEmpty: {
		...
		LangJapanese:           "プロバイダが設定されていません。\n\n`config.toml` または `cf-connect provider add` で追加してください。",
	...
	MsgStatusTitle: {
		...
		LangChinese: "cf-connect 状态\n\n" +
			...
			"语言: %s\n" +
			"%s" + "%s" + "%s" + "%s" + "%s" + "%s",
		LangTraditionalChinese: "cf-connect 狀態\n\n" +
			"項目: %s\n" +
			"Agent: %s\n" +
			"工作目錄: %s\n" +
			"平台: %s\n" +
		...
		LangJapanese: "cf-connect ステータス\n\n" +
			...
			"言語: %s\n" +
			"%s" + "%s" + "%s" + "%s" + "%s" + "%s",
		LangSpanish: "Estado de cf-connect\n\n" +
			"Proyecto: %s\n" +
			"Agente: %s\n" +
			"Directorio: %s\n" +
			"Plataformas: %s\n" +
	...
	MsgCardTitleStatus: {
		LangEnglish: "cf-connect Status", LangChinese: "cf-connect 状态", LangTraditionalChinese: "cf-connect 狀態",
		LangJapanese: "cf-connect ステータス", LangSpanish: "Estado de cf-connect",
	},
	...
	MsgRestarting: {
		LangEnglish:            "🔄 Restarting cf-connect...",
		LangChinese:            "🔄 正在重启 cf-connect...",
		LangTraditionalChinese: "🔄 正在重啟 cf-connect...",
		LangJapanese:           "🔄 cf-connect を再起動中...",
		LangSpanish:            "🔄 Reiniciando cf-connect...",
	},
	MsgRestartSuccess: {
		LangEnglish:            "✅ cf-connect restarted successfully.",
		LangChinese:            "✅ cf-connect 重启成功。",
		LangTraditionalChinese: "✅ cf-connect 重啟成功。",
		LangJapanese:           "✅ cf-connect の再起動が完了しました。",
		LangSpanish:            "✅ cf-connect se reinició correctamente.",
	},
	MsgUpgradeChecking: {
	...
	MsgRelaySetupHint: {
		LangEnglish:            "\n\n⚠️ This agent does not auto-inject cf-connect instructions.\nPlease run `/bind setup` or `/cron setup` to write instructions to %s.",
		LangChinese:            "\n\n⚠️ 当前 agent 不会自动注入 cf-connect 指令。\n请运行 `/bind setup` 或 `/cron setup` 将指令写入 %s。",
		LangTraditionalChinese: "\n\n⚠️ 當前 agent 不會自動注入 cf-connect 指令。\n請執行 `/bind setup` 或 `/cron setup` 將指令寫入 %s。",
		LangJapanese:           "\n\n⚠️ このエージェントは cf-connect の指示を自動注入しません。\n`/bind setup` または `/cron setup` を実行して %s に指示を書き込んでください。",
		LangSpanish:            "\n\n⚠️ Este agente no inyecta automáticamente las instrucciones de cf-connect.\nEjecute `/bind setup` o `/cron setup` para escribirlas en %s.",
	},
	MsgRelaySetupOK: {
		LangEnglish:            "✅ cf-connect instructions written to %s\nThe agent can now use relay, cron, and attachment send-back.",
		LangChinese:            "✅ cf-connect 指令已写入 %s\nagent 现在可以使用中继、定时任务和附件回传功能了。",
		LangTraditionalChinese: "✅ cf-connect 指令已寫入 %s\nagent 現在可以使用中繼、定時任務和附件回傳功能了。",
		LangJapanese:           "✅ cf-connect の指示を %s に書き込みました。\nエージェントがリレー、cron、添付ファイル返送を使えるようになりました。",
		LangSpanish:            "✅ Instrucciones de cf-connect escritas en %s\nEl agente ahora puede usar relay, cron y reenvío de adjuntos.",
	},
	MsgRelaySetupExists: {
		LangEnglish:            "ℹ️ cf-connect instructions already exist in %s — no changes made.",
		LangChinese:            "ℹ️ cf-connect 指令已存在于 %s 中，无需重复写入。",
		LangTraditionalChinese: "ℹ️ cf-connect 指令已存在於 %s 中，無需重複寫入。",
		LangJapanese:           "ℹ️ cf-connect の指示は既に %s に存在します。変更はありません。",
		LangSpanish:            "ℹ️ Las instrucciones de cf-connect ya existen en %s — sin cambios.",
	},
	...
	MsgSetupNative: {
		LangEnglish:            "✅ This agent natively supports cf-connect instructions — no setup needed.",
		LangChinese:            "✅ 当前 agent 已原生支持 cf-connect 指令，无需额外配置。",
		LangTraditionalChinese: "✅ 當前 agent 已原生支持 cf-connect 指令，無需額外配置。",
		LangJapanese:           "✅ このエージェントは cf-connect の指示をネイティブサポートしています。セットアップ不要です。",
		LangSpanish:            "✅ Este agente soporta nativamente las instrucciones de cf-connect — no se necesita configuración.",
	},
	MsgCronSetupOK: {
		LangEnglish:            "✅ cf-connect instructions written to %s\nThe agent can now use relay, cron, and attachment send-back.",
		LangChinese:            "✅ cf-connect 指令已写入 %s\nagent 现在可以使用中继、定时任务和附件回传功能了。",
	...
	MsgBuiltinCmdRestart: {
		LangEnglish:            "Restart cf-connect service",
		LangChinese:            "重启 cf-connect 服务",
		LangTraditionalChinese: "重啟 cf-connect 服務",
		LangJapanese:           "cf-connect サービスを再起動",
		LangSpanish:            "Reiniciar el servicio cf-connect",
	},
	MsgBuiltinCmdStatus: {
	...
	MsgBuiltinCmdVersion: {
		LangEnglish:            "Show cf-connect version",
		LangChinese:            "查看 cf-connect 版本",
		LangTraditionalChinese: "查看 cf-connect 版本",
		LangJapanese:           "cf-connect のバージョンを表示",
		LangSpanish:            "Mostrar versión de cf-connect",
	},
	MsgBuiltinCmdHelp: {
...
When you generate a local image or file that should be sent to the user, use:
  ...
  cf-connect send --image /absolute/path/to/image.png
...
When sending an audio (mp3/wav/m4a/ogg/opus) or video (mp4/mov/webm) clip that should render inline as a native voice bubble or video player — instead of as a generic file download — use the dedicated flags:
  ...
  cf-connect send --audio /absolute/path/to/clip.mp3
  cf-connect send --video /absolute/path/to/demo.mp4

These render as native media on platforms that support it (e.g. Feishu voice bubbles, Telegram voice messages). cf-connect transparently transcodes audio to the platform's preferred codec (e.g. opus for Feishu). On platforms without dedicated audio/video support cf-connect automatically falls back to the file-attachment path so delivery is preserved. Do NOT downgrade the user's request to --file when they explicitly asked for audio or video.

When the user explicitly asks you to synthesize speech from text, use:

  cf-connect send --tts "text to speak"

After cf-connect send --tts (or --audio) succeeds, reply only with NO_REPLY unless the user also asked for a visible text confirmation. This prevents sending an extra text message after the voice message.`,
...
这些参数会在支持原生媒体的平台上渲染为原生形态(例如飞书的语音气泡、Telegram 的语音消息)。cf-connect 会自动把音频转码为平台偏好的编码(例如飞书的 opus)。在不支持专用音视频的平台,cf-connect 会自动回退到文件附件路径以保证投递成功。当用户明确要求 audio/video 时,不要把请求降级为 --file。

当用户明确要求把文字合成为语音时,使用:

  cf-connect send --tts "要朗读的文字"

cf-connect send --tts(或 --audio)成功之后,除非用户同时要求可见的文字确认,否则只回复 NO_REPLY,避免在语音消息后再发一条文字消息。`,
	},
	MsgAgentCronToolPrompt: {
		LangEnglish: `### Scheduled tasks: when to use /cron vs /timer

cf-connect has TWO distinct scheduling commands. Picking the wrong one creates a confusing UX for the user.

  ┌──────────────────────────────┬─────────────────────────────┐
  │ Use cf-connect cron …        │ Use cf-connect timer …      │
  ├──────────────────────────────┼─────────────────────────────┤
...
Examples:
  ...
  cf-connect cron add --cron "*/2 * * * *" --exec "ipconfig" --session-mode new-per-run --desc "Every 2 min ipconfig"

You can also list, inspect, run, edit, or delete cron jobs:
  cf-connect cron list
  cf-connect cron info <job-id> [field]
  cf-connect cron exec <job-id>
  cf-connect cron edit <job-id> <field> <value>
  cf-connect cron del <job-id>

When changing an existing job, first run ` + "`cf-connect cron info <job-id>`" + ` to inspect the current values, then use ` + "`cron edit`" + ` for only the field(s) the user asked to change.
...
cf-connect 有两个不同的调度命令。选错会让用户感到很困惑。

  ┌──────────────────────────────┬─────────────────────────────┐
  │ 使用 cf-connect cron …       │ 使用 cf-connect timer …     │
  ├──────────────────────────────┼─────────────────────────────┤
  │ 周期性调度                    │ 单次延时 / 一次性任务         │
  │ "每天/每周/每小时"            │ "X 分钟后/小时后/明天"        │
  │ "every day/week/Monday"      │ "in 30 min", "tomorrow 9am"  │
...
当用户让你做周期性任务时(例如"每天早上6点帮我总结GitHub trending"),用 Bash 工具执行:
  ...
  cf-connect cron add --cron "<分> <时> <日> <月> <星期>" --prompt "<任务描述>" --desc "<简短标签>"
...
示例:
  cf-connect cron add --cron "0 6 * * *" --prompt "汇总 GitHub trending 仓库并发摘要" --desc "每日 GitHub Trending"
  cf-connect cron add --cron "0 9 * * 1" --prompt "生成本周项目状态报告" --desc "每周报告"
  cf-connect cron add --cron "*/2 * * * *" --exec "ipconfig" --session-mode new-per-run --desc "每 2 分钟跑 ipconfig"

你也可以列出、检查、立即执行、修改或删除 cron 任务:
  cf-connect cron list
  cf-connect cron info <job-id> [字段]
  cf-connect cron exec <job-id>
  cf-connect cron edit <job-id> <字段> <新值>
  cf-connect cron del <job-id>

修改现有任务时,先用 ` + "`cf-connect cron info <job-id>`" + ` 看当前值,再用 ` + "`cron edit`" + ` 只改用户要求改的字段。
...
完整字段列表见 ` + "`cf-connect cron edit --help`" + `。

示例:
  cf-connect cron exec abc123
  cf-connect cron edit abc123 cron_expr "0 9 * * *"
  cf-connect cron edit abc123 enabled false
  cf-connect cron edit abc123 prompt "更新后的每日摘要任务"`,
	},
...
use the Bash tool to run:
  ...
  cf-connect timer add --delay <duration> --prompt "<task description>"
...
Examples:
  cf-connect timer add --delay 2h --prompt "Check PR status" --desc "PR check"
  cf-connect timer add --delay 30m --exec "df -h" --desc "Disk check"
  cf-connect timer add --at "2026-05-16T09:00" --prompt "Morning standup reminder"

You can also list or cancel timers:
  cf-connect timer list
  cf-connect timer del <timer-id>`,
		LangChinese: `### 一次性延时 (timer) — ONE-TIME DELAY
当用户让你在一段延时之后或在某个未来时刻做某事时
(例如"两小时后帮我检查PR"、"3 分钟后看下系统负载"、"明天早上 9 点提醒我"),
用 Bash 工具执行:

  cf-connect timer add --delay <时长> --prompt "<任务描述>"
...
示例:
  cf-connect timer add --delay 2h --prompt "检查 PR 状态" --desc "PR 检查"
  cf-connect timer add --delay 30m --exec "df -h" --desc "磁盘检查"
  cf-connect timer add --at "2026-05-16T09:00" --prompt "早会提醒"
...
When you need to communicate with another bot (e.g. ask another AI agent a question), use:
  ...
  cf-connect relay send --to <target_project> "<message>"
...
当需要和另一个 bot 通信(例如向另一个 AI agent 提问)时,使用:
  ...
  cf-connect relay send --to <目标项目名> "<消息>"
