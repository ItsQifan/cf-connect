...
// NOTE: the Feishu/Lark, Weixin (ilink) and Yuanbao credential-provisioning
// helpers (EnsureProjectWith*Platform, Save*PlatformCredentials and their
// option/result types) were removed together with those platforms. Config
// writing for platforms now goes through AddPlatformToProject.

// NewProjectAgentOptions controls how the agent block of an auto-provisioned
// project is derived.
type NewProjectAgentOptions struct {
	...
	AgentType        string // optional default agent type when no source project exists
}

func pickAgentTemplateForNewProject(cfg *Config, opts NewProjectAgentOptions) AgentConfig {
	...
	if agentType := strings.TrimSpace(opts.AgentType); agentType != "" {
		realType, _, _ := strings.Cut(agentType, ":")
		return AgentConfig{
			Type:    realType,
