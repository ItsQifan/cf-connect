...
func TestBuildPlist_KeepAliveDoesNotRestartOnCleanExit(t *testing.T) {
	cfg := Config{
		BinaryPath: "/opt/cf-connect/cf-connect",
		WorkDir:    "/tmp/wd",
		LogFile:    "/tmp/log",
		LogMaxSize: 10485760,
		EnvPATH:    "/usr/bin",
	...
	// (exit 0) exit; false means restart ONLY after an unsuccessful exit. cf-connect
...
func TestBuildPlist_EscapesXMLSpecialCharsInPaths(t *testing.T) {
	cfg := Config{
		BinaryPath: "/opt/cf-connect/bin & <tools>/cf-connect",
		WorkDir:    "/Users/jane/Projects/dev & test/cf-connect",
...
// path: a user from an earlier cf-connect version may already have a
