...
func TestBuildWindowsTaskScript(t *testing.T) {
	cfg := Config{
		BinaryPath: `C:\Program Files\cf-connect\cf-connect.exe`,
		WorkDir:    `C:\Users\me\.cf-connect`,
		LogFile:    `C:\Users\me\.cf-connect\logs\cf-connect.log`,
	...
	for _, want := range []string{
		`$env:CC_LOG_FILE = 'C:\Users\me\.cf-connect\logs\cf-connect.log'`,
		`$env:CC_LOG_MAX_SIZE = '10485760'`,
		`$env:PATH = 'C:\Program Files\nodejs;C:\Users\me\AppData\Local\Programs'`,
		`$env:HTTPS_PROXY = 'http://127.0.0.1:7890'`,
		`$env:http_proxy = 'http://127.0.0.1:7890'`,
		`Set-Location -LiteralPath 'C:\Users\me\.cf-connect'`,
		`while ($true) {`,
		`& 'C:\Program Files\cf-connect\cf-connect.exe'`,
...
func TestWindowsTaskActionRunsHidden(t *testing.T) {
	got := windowsTaskAction(`C:\Users\me\.cf-connect\cf-connect-daemon.ps1`)
	for _, want := range []string{
		`powershell.exe`,
		`-WindowStyle Hidden`,
		`-NoProfile`,
		...
		`-File "C:\Users\me\.cf-connect\cf-connect-daemon.ps1"`,
	} {
...
func TestWindowsTaskCreateUsesLimitedInteractivePrincipal(t *testing.T) {
	...
	if err := createWindowsTask(`C:\Users\me\.cf-connect\cf-connect-daemon.ps1`); err != nil {
	...
	for _, want := range []string{
		...
		`-RunLevel Limited`,
		`-WindowStyle Hidden`,
		`C:\Users\me\.cf-connect\cf-connect-daemon.ps1`,
	} {
		if !strings.Contains(script, want) {
...
func TestWindowsTaskMatchesActionRequiresExactAction(t *testing.T) {
	...
	if !windowsTaskMatchesAction(`C:\Users\me\.cf-connect\cf-connect-daemon.ps1`) {
		t.Fatal("windowsTaskMatchesAction() = false, want true")
	}
	for _, want := range []string{
		`$expectedArgs = '-WindowStyle Hidden -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "C:\Users\me\.cf-connect\cf-connect-daemon.ps1"'`,
...
func TestPowerShellLiteralEscapesSingleQuotes(t *testing.T) {
	got := powerShellLiteral(`C:\Users\O'Brien\.cf-connect`)
	want := `'C:\Users\O''Brien\.cf-connect'`
	if got != want {
...
// mode of a script left by an earlier cf-connect version. While
