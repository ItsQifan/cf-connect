...
```
   ...
   user → MAX cloud ─── HTTPS POST ───▶  │  nginx :443 (TLS)                     │
                       ...
                       /webhook           │                                       │
                                          │  cf-connect (HTTP :8090, localhost)   │
                                          └───────────────────────────────────────┘
```
...
```
                                          ┌─── VPS ───┐         ┌──── Home ────┐
   user → MAX cloud ─── HTTPS ─────────▶ │  nginx    │ ──SSH──▶│  cf-connect  │
...
3. `systemctl restart cf-connect` (or however you run it).
...
1. `systemctl --user status cf-connect` — is it running?
