...
Enable a single cf-connect bot (one Slack token) to serve multiple workspaces, with the channel determining which Claude Code working directory and session to use.
...
Persisted in `~/.cf-connect/workspace_bindings.json`:
