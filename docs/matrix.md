...
This guide walks you through connecting **cf-connect** to [Matrix](https://matrix.org/), the open standard for decentralized communication. Once set up, you can chat with your local AI agent from any Matrix client (Element, FluffyChat, Nheko, etc.).

## Prerequisites

- A Matrix account on any homeserver (public like `matrix.org`, or self-hosted)
- A machine that can run cf-connect (no public IP needed)
...
You need an access token so cf-connect can authenticate as your Matrix user.
...
curl -XPOST "https://matrix.org/_matrix/client/v3/login" \
  ...
  -d '{"type":"m.login.password","user":"your-username","password":"your-password","device_id":"CF-Connect"}'
```

The response contains `"access_token": "syt_..."`. Copy it for the config.

> **Recommended**: Set `device_id` to `CF-Connect` or another recognizable name. A dedicated device ensures encryption keys are distributed correctly.
...
Your user ID looks like `@username:matrix.org`. cf-connect can auto-detect it from the access token, but you can also specify it explicitly in config.

In Element: click your avatar — your user ID is shown at the top.

---

## Step 4: Configure cf-connect
...
```
level=INFO msg="matrix: E2EE enabled" device_id=CF-Connect
...
3. Send a message — cf-connect will respond
...
1. Is cf-connect running and showing `matrix: connected` in logs?
...
Make sure `auto_join = true` (this is the default). If the bot was already invited before cf-connect started, re-invite it.

### Q: E2EE (End-to-End Encryption)

cf-connect supports encrypted rooms (E2EE) when built with the `goolm` build tag. If you see `matrix: E2EE enabled` at startup, encryption is working. If you see `matrix: E2EE not available (build with -tags goolm to enable)`, rebuild with E2EE support:

> **Data storage**: E2EE crypto data is stored under `~/.cf-connect/` (created with `0700` permissions):
...
>
> To reset E2EE (e.g. after changing device or reinstalling), delete these files and restart cf-connect:
> ```bash
> rm ~/.cf-connect/matrix-crypto-*.db* ~/.cf-connect/matrix-cross-signing-*.json
> ```

```bash
go build -tags goolm ./cmd/cf-connect
```

> **Note**: To remove the red question mark ("encrypted by a device not verified by its owner") on bot messages, cross-signing must be set up. cf-connect does this automatically on first run, but some servers require `cross_signing_password` in config for the initial setup. See the red question mark FAQ below.
...
2. **`not marked as shared, but there are keys on the server`** — Old crypto data conflicts with the current device. cf-connect tries to auto-recover. If it persists, delete old crypto databases and cross-signing seeds: `rm ~/.cf-connect/matrix-crypto-*.db* ~/.cf-connect/matrix-cross-signing-*.json`
3. **`mismatching device ID in client and crypto store`** — The token's device ID doesn't match the crypto database. Delete the database and seeds: `rm ~/.cf-connect/matrix-crypto-*.db* ~/.cf-connect/matrix-cross-signing-*.json`
...
3. **Corrupted crypto database** — Delete and restart: `rm ~/.cf-connect/matrix-crypto-*.db* ~/.cf-connect/matrix-cross-signing-*.json`
...
curl -XPOST "https://your-homeserver.com/_matrix/client/v3/login" \
  ...
  -d '{
    ...
    "device_id": "CF-Connect"
  }'
```

The `access_token` in the response can be used in config. The `device_id` will be `CF-Connect`, easy to identify and manage.

#### Red question mark on bot messages ("encrypted by a device not verified by its owner")?

This means the bot's device hasn't been cross-signed. cf-connect automatically sets up cross-signing on first run, but some Matrix servers require password authentication (UIA) to publish the cross-signing keys.
...
cf-connect auto-accepts SAS key verification requests (when `auto_verify = true`, which is the default). To verify from Element:
...
Set `homeserver` to your server's URL (e.g. `https://synapse.example.com`). Make sure the URL is reachable from the machine running cf-connect.
