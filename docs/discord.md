...
This guide walks you through connecting **cf-connect** to Discord, so you can chat with your local Claude Code via a Discord bot.
...
2. Enter an application name (e.g. `cf-connect`)
...
> cf-connect automatically configures the required Intents (MESSAGE_CONTENT, GUILD_MESSAGES, DIRECT_MESSAGES).
> With `thread_isolation = true`, cf-connect creates or reuses a Discord thread for each session and routes follow-up messages by thread channel ID.
...
```
level=INFO msg="discord: connected" bot=cf-connect#0000
...
cf-connect: 🤔 Thinking...
cf-connect: 🔧 Tool: Bash(ls -la)
cf-connect: Here's the project structure...
```
...
| **Message limit** | 2000 characters per message (auto-split by cf-connect) |
