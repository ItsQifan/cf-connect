...
cf-connect provider import --project my-backend  # 从 cc-switch 导入
```
...
- 修改 `config.toml` 后，需要重启 `cf-connect`。
- 目录切换会作用于当前项目的下一次会话。
...
cf-connect weixin setup --project my-project --token '<token>'
...
常用参数：`--api-url`、`--cdn-url`、`--timeout`、`--qr-image`、`--route-tag`、`--bot-type`、`--debug`（详见 `cf-connect weixin help` 或 [weixin.md](./weixin.md)）。
...
如果你在 Claude Code 的 `settings.json` 中配置了 [PermissionRequest hooks](https://docs.anthropic.com/en/docs/claude-code/hooks)，cf-connect 会读取并执行它们——匹配的 hook 可以在请求到达消息平台之前自动批准或拒绝。

### 为什么 hook 会被执行两次

cf-connect 使用 `--permission-prompt-tool stdio` 模式启动 Claude Code，这意味着 Claude Code 自身执行 hook 的输出会被丢弃（stdout 被协议占用）。为了让你的 hook 真正生效，cf-connect 会从 `settings.json` 中读取 hook 定义，然后**独立重新执行一次**。

也就是说，每次权限请求你的 hook 命令会被执行**两次**：

1. Claude Code 执行一次（结果丢弃）
2. cf-connect 执行一次（结果生效）
...
if [ -n "$CC_CONNECT_PERMISSION_HOOK_SKIP" ]; then
  exit 0  # cf-connect 会在不含此变量的环境下重新执行我们
fi
# ... 你的 hook 逻辑 ...
```

cf-connect 启动 Claude Code 子进程时会在环境中设置 `CC_CONNECT_PERMISSION_HOOK_SKIP=1`。当你的 hook 检测到这个变量时，说明它运行在 Claude Code 内部（结果会被丢弃）——跳过昂贵的逻辑即可。cf-connect 在自己执行 hook 时会剥离这个变量，所以第二次执行会正常运行。
...
当 Agent 在本地生成了图片、PDF、日志包、报表等文件，需要把结果直接发回当前聊天时，可以使用 `cf-connect send` 的附件模式。用户明确要求“发语音”时，Agent 也可以用同一个 CLI 走 TTS 合成并发送语音。
...
这两个命令写入的是同一份 cf-connect 指令。执行任意一个即可。这样 Agent 才会知道：
- 普通文本回复直接正常输出
- 生成附件后用 `cf-connect send --image/--file` 回传
- 用户要求语音时用 `cf-connect send --tts` 回传
...
默认值是 `on`。这个开关与 agent 的 `/mode` 独立，只影响 `cf-connect send --image/--file` 这条图片/文件回传路径。TTS 语音回传走 `[tts]` provider 配置，由 TTS 是否可用决定。
...
- 每个附件默认上限 **50 MiB**。可在 config.toml 用 `max_attachment_size_mb`（单位 MiB）调整，或用环境变量 `CC_MAX_ATTACHMENT_SIZE_MB` 覆盖该值（同样单位 MiB，设置后优先级更高），例如 `CC_MAX_ATTACHMENT_SIZE_MB=100 cf-connect send --file big.bin`。
...
- 目标平台在投递时还会校验自己的文件大小/类型上限；实际生效的是它与 `max_attachment_size_mb` 中**更小**的那个（通过了 cf-connect 的文件仍可能在投递时被平台拒绝）。
...
```bash
cf-connect cron add --cron "0 6 * * *" --prompt "总结 GitHub trending" --desc "每日趋势"
cf-connect cron list
cf-connect cron edit <job-id> <field> <value>   # 可改 cron_expr / prompt / enabled / mute / timeout_mins 等
...
默认情况下，cf-connect 在 Unix 上使用 `sh`，Windows 上使用 `powershell.exe` 来执行所有 shell 命令（`/shell`、cron exec、hooks 和 webhook exec）。你可以配置使用其他 shell。
...
Shell 配置适用于 cf-connect 中所有命令执行路径：
...
```bash
cf-connect relay send --to gemini "你觉得这个架构怎么样？"
```
...
| `POST` | `/api/v1/restart` | 重启 cf-connect |
...
Bridge 提供 WebSocket + REST 服务，让外部适配器（自定义 UI、机器人、脚本等）可以接入 cf-connect —— 发送消息、接收 Agent 事件、管理会话。
...
**远端 Gateway 必须先完成配对授权。** 如果你把 cf-connect 指向
...
4. 完成后 cf-connect 才能与已授权的 Gateway 通信
...
1. 启动 cf-connect，并让机器人加入目标群。
2. 让群里任意已知允许的用户在群里发一条消息。
3. 查看 cf-connect 日志，消息到达时会打印带 `@chatroom` 后缀的
