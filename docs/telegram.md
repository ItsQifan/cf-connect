...
This guide walks you through connecting **cf-connect** to Telegram, so you can chat with your local Claude Code via a Telegram bot.
...
Enter a **display name** for your bot (e.g. `cf-connect`).
...
Telegram topics include a `message_thread_id`. cf-connect uses that thread ID
...
Check the following:
1. Is cf-connect running?
2. Is the bot token correct?
3. Have you sent a message after starting cf-connect? (The bot only receives messages after startup)
