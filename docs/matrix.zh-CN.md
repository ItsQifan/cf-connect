...
本指南将帮助你把 **cf-connect** 连接到 [Matrix](https://matrix.org/)——一个去中心化通信的开放标准。配置完成后，你可以通过任何 Matrix 客户端（Element、FluffyChat、Nheko 等）与本地 AI 编码助手对话。

## 前提条件

- 一个 Matrix 账号（可使用 `matrix.org` 等公共服务器，也可自建）
- 一台可以运行 cf-connect 的机器（无需公网 IP）
...
你需要一个 access token 让 cf-connect 以你的 Matrix 用户身份进行认证。
...
> **推荐**：将 `device_id` 设为 `CF-Connect` 或其他易于识别的名称。专用设备能确保加密密钥正确分发。
...
你的用户 ID 格式为 `@username:matrix.org`。cf-connect 可以从 access token 自动检测，但你也可以在配置中显式指定。
...
1. cf-connect 是否在运行，且日志中显示 `matrix: connected`？
...
确保 `auto_join = true`（这是默认值）。如果机器人在 cf-connect 启动之前就被邀请了，请重新邀请一次。

### E2EE（端到端加密）

cf-connect 需要使用 `goolm` build tag 编译才能支持加密房间（E2EE）。启动时如果看到 `matrix: E2EE enabled`，说明加密功能正常。如果看到 `matrix: E2EE not available (build with -tags goolm to enable)`，需要重新编译：

> **数据存储**：E2EE 加密数据存储在 `~/.cf-connect/` 目录下（目录权限为 `0700`）：
> - `matrix-crypto-<device_id>.db` — 加密密钥数据库（每个设备一个）
> - `matrix-cross-signing-<device_id>.json` — 跨签名种子文件（每个设备一个）
>
> 如需重置 E2EE（例如更换设备或重新安装），删除这些文件后重启 cf-connect 即可：
> ```bash
...
> **注意**：要消除机器人消息上的红问号（"由未经其所有者验证的设备加密"），需要完成跨签名设置。cf-connect 会在首次运行时自动设置，但部分服务器需要在配置中设置 `cross_signing_password`。详见下方红问号相关常见问题。
...
2. **`not marked as shared, but there are keys on the server`** — 旧的加密数据与当前设备冲突。cf-connect 会自动尝试修复。如果持续失败，删除旧的加密数据库和跨签名种子：`rm ~/.cf-connect/matrix-crypto-*.db* ~/.cf-connect/matrix-cross-signing-*.json`
3. **`mismatching device ID in client and crypto store`** — token 对应的 device ID 与加密数据库不匹配。删除数据库和种子文件：`rm ~/.cf-connect/matrix-crypto-*.db* ~/.cf-connect/matrix-cross-signing-*.json`
...
3. **加密数据库损坏** — 删除数据库和种子文件后重启：`rm ~/.cf-connect/matrix-crypto-*.db* ~/.cf-connect/matrix-cross-signing-*.json`
...
响应中的 `access_token` 即可用于配置。`device_id` 为 `CF-Connect`，便于识别和管理。

#### 机器人消息有红问号（"由未经其所有者验证的设备加密"）？

这说明机器人的设备未被跨签名。cf-connect 首次运行时会自动设置跨签名，但部分 Matrix 服务器需要密码认证（UIA）才能发布跨签名密钥。
...
cf-connect 会自动接受 SAS 密钥验证请求（`auto_verify = true`，默认开启）。从 Element 验证机器人的步骤：
...
将 `homeserver` 设为你的服务器 URL（如 `https://synapse.example.com`）。确保运行 cf-connect 的机器可以访问该 URL。
